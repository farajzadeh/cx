...
complete -c cx -n __fish_use_subcommand -a peek      -d 'what each session is doing now'
complete -c cx -n __fish_use_subcommand -a nudge     -d 'send a prompt to a running session'
complete -c cx -n __fish_use_subcommand -a bar       -d 'one line for a tmux status bar'
complete -c cx -n __fish_use_subcommand -a tabs      -d 'a tmux tab per live session, here'
complete -c cx -n __fish_use_subcommand -a jump      -d 'go to the tab of the session that needs you'
complete -c cx -n __fish_use_subcommand -a goal      -d 'definitions of done for your sessions'
complete -c cx -n __fish_use_subcommand -a driver    -d 'print the cx-driver subagent definition'
...
complete -c cx -n __fish_use_subcommand -a forget    -d 'drop a finished session from the lists'
...
complete -c cx -n '__fish_seen_subcommand_from open resume shell code ask peek nudge stop rm forget' -a '(__cx_targets)'
complete -c cx -n '__fish_seen_subcommand_from goal' -a 'new ls show dod member pause resume done log on-stop rm'
...
complete -c cx -n '__fish_seen_subcommand_from tabs' -l dry-run -s n -d 'show what it would open'
complete -c cx -n '__fish_seen_subcommand_from tabs' -l no-attach -d 'build it, do not attach'
complete -c cx -n '__fish_seen_subcommand_from tabs' -l session -s s -d 'tmux session name'
complete -c cx -n '__fish_seen_subcommand_from tabs' -l take -d 'open sessions held elsewhere too'
complete -c cx -n '__fish_seen_subcommand_from jump' -l states -d 'which states to go to'
complete -c cx -n '__fish_seen_subcommand_from wt worktree' -l merged -d 'every merged worktree'
